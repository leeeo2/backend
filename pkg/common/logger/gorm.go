...
func NewGormLogger(logConf *Config, gormLogConf *GormLogConfig) (gormlog.Interface, error) {
	c := *logConf
	level := strToGormLogLevel(gormLogConf.GormLevel)
	...
	return &GormLogger{
		logger:                    l,
		level:                     level,
		slowThreshold:             gormLogConf.SqlSlowThreshold,
		ignoreRecordNotFoundError: gormLogConf.IgnoreRecordNotFoundError,
		ignoreDuplicateError:      gormLogConf.IgnoreDuplicateError,
	}, nil
